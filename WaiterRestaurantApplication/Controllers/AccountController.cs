...
namespace WaiterRestaurantApplication.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            ViewBag.Name = new SelectList(context.Roles.Where(u => !u.Name.Contains("Admin") && !u.Name.Contains("Diner"))
                    .ToList(), "Name", "Name");
            return View();
        }
        ...
        public Boolean isRestaurantManagerUser()
        {
            if (User.Identity.IsAuthenticated)
            {
                var user = User.Identity;
                ...
                var UserManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(context));
                var s = UserManager.GetRoles(user.GetUserId());
                if (s[0].ToString() == "RestaurantManager")
                {
                    return true;
                }
                else
        ...
        public Boolean isRestaurantEmployeeUser()
        {
            if (User.Identity.IsAuthenticated)
            {
                var user = User.Identity;
                ...
                var UserManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(context));
                var s = UserManager.GetRoles(user.GetUserId());
                if (s[0].ToString() == "RestaurantEmployee")
                {
                    return true;
                }
                else
